...
import type { ResolvedWebSocketRoute } from "../http/router";
import type { NodeUpgradeSocket, WebSocketVerificationResult } from "../shared/types";
...
} from "./permessage-deflate";
import { createWebSocket, KenoWebSocket } from "./connection";
...
export async function upgradeWebSocket(
...
): Promise<KenoWebSocket | undefined> {
  ...
  if (!connection || !connection.toLowerCase().split(/\s*,\s*/u).includes("upgrade")) {
  ...
  if (version !== "13") {
    rejectUpgrade(socket, 400, "Missing or invalid Sec-WebSocket-Version header", {
  ...
  const acceptValue = createHash("sha1").update(key + GUID).digest("base64");
  const headers = [
  ...
  if (session.perMessageDeflate) {
    headers.push(`Sec-WebSocket-Extensions: ${perMessageDeflateResponseHeader()}`);
  }

  for (const [name, value] of Object.entries(sanitizeHttp1Headers(session.extraHeaders))) {
    headers.push(`${name}: ${value}`);
  }

  socket.write(`${headers.join("\r\n")}\r\n\r\n`);

  if (head.byteLength > 0 && "unshift" in socket && typeof socket.unshift === "function") {
    socket.unshift(head);
  }

  return createWebSocket(socket, {
    autoPong: route.options.autoPong ?? true,
    maxPayload: parseSize(route.options.maxPayload, 1024 * 1024),
...
export async function acceptExtendedConnectWebSocket(
  ...
  if (version !== "13") {
    rejectExtendedConnect(stream, 400, "Missing or invalid Sec-WebSocket-Version header", {
      "sec-websocket-version": "13",
    });
    return undefined;
  }
...
async function prepareWebSocketSession(
  ...
  if (route.options.verifyClient) {
    const result = normalizeVerificationResult(await route.options.verifyClient(request as never));
  ...
  const requestedProtocols = parseProtocols(request.get("sec-websocket-protocol"));
...
async function selectProtocol(
  ...
  if (route.options.handleProtocols) {
    const selected = await route.options.handleProtocols(requestedProtocols, request as never);

    if (selected && !requestedProtocols.has(selected)) {
      throw new TypeError(`Selected WebSocket protocol "${selected}" was not requested by the client`);
    }
  ...
  if (route.options.protocols && route.options.protocols.length > 0) {
    return route.options.protocols.find((protocol) => requestedProtocols.has(protocol));
  }

  return requestedProtocols.values().next().value;
}

function sanitizeHttp2Headers(headers: Record<string, string>): IncomingHttpHeaders {
  const sanitized: IncomingHttpHeaders = {};

  for (const [name, value] of Object.entries(headers)) {
    if (!isSafeHeader(name, value) || HTTP2_FORBIDDEN_RESPONSE_HEADERS.has(name.toLowerCase())) {
      continue;
    }
...
function sanitizeHttp1Headers(headers: Record<string, string>): Record<string, string> {
...
function isSafeHeader(name: string, value: string): boolean {
  return HEADER_NAME_PATTERN.test(name) && !INVALID_HEADER_VALUE_PATTERN.test(value);
}
